...
namespace Xamarin.UITest.Queries
{
    ...
    /// </summary>
    public class PropertyAppQuery : IFluentInterface
    {
        ...
        {
            return BuildPredicate(new SingleQuoteEscapedString(text), "BEGINSWITH", "StartsWith");
        }
        ...
        {
            return BuildPredicate(new SingleQuoteEscapedString(text), "ENDSWITH", "EndsWith");
        }
        ...
        {
            return BuildPredicate(new SingleQuoteEscapedString(text), "CONTAINS", "Contains");
        }
        ...
        {
            return BuildPredicate(new SingleQuoteEscapedString(text), "LIKE", "Like");
        }
        ...
        {
            return BuildPredicate(new SingleQuoteEscapedString(text), predicate, "Predicate");
        }

        AppQuery BuildPredicate(SingleQuoteEscapedString text, string predicate, string methodName)
        {
            var queryString = string.Format("{{{0} " + predicate + " '{1}'}}", _propertyName, text);
            var codeString = string.Format("Property(\"{0}\").{1}(\"{2}\")", _propertyName, methodName, text.UnescapedString);

            return new AppQuery(_appQuery, new RawToken(queryString, codeString));
        }
